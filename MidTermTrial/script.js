"use strict";
let authorsData = []; // Store all authors for filtering and sorting
let sortField = "id";
...
const getYearsActive = (bibliography, deathDate) => {
...
const summarizeBibliography = (bibliography) => {
...
const populateNationalities = (authors) => {
    const nationalities = Array.from(new Set(authors.map((author) => author.nationality))).sort();
    const dropdown = document.getElementById("filter-nationality");
...
const filterAuthors = () => {
    ...
    const aliveFilter = document.getElementById("filter-alive").checked;
    const yearActiveFilter = parseInt(document.getElementById("filter-year-active").value);
    return authorsData.filter((author) => {
        ...
        const matchesYearActive = isNaN(yearActiveFilter) ||
...
const renderAuthors = (authors) => {
    ...
    sortedAuthors.forEach((author) => {
      ...
      <div class="author-data">${new Date(author.birth_date).toISOString().split("T")[0]}</div>
      <div class="author-data"><input type="checkbox" disabled ${!author.death_date ? "checked" : ""}></div>
